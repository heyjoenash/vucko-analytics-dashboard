-- Create view for people with campaign source information
-- This view enriches person data with campaign attribution
...
-- Grant permissions
GRANT SELECT ON people_with_campaign_source TO authenticated;

-- Create index for better performance (on base tables)
CREATE INDEX IF NOT EXISTS idx_posts_linkedin_campaign_id ON posts(linkedin_campaign_id);
CREATE INDEX IF NOT EXISTS idx_engagements_post_person ON engagements(post_id, person_id);

-- Log successful migration
DO $$
BEGIN
    RAISE NOTICE 'People Campaign Source View Migration Completed Successfully';
    RAISE NOTICE 'Created people_with_campaign_source view for campaign attribution';
    RAISE NOTICE 'Added indexes for improved query performance';
    RAISE NOTICE 'People view now shows campaign source indicators and acquisition costs';
END $$;